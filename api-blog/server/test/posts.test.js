...
test('Should get posts', async ()=>{
    ...
    //given - dado que
    const post1 = await  postService.create({title: generateRandomText(10), content: generateRandomText(30)})
    const post2 = await postService.create({title: generateRandomText(10), content: generateRandomText(30)})
    const post3 = await postService.create({title: generateRandomText(10), content: generateRandomText(30)})
    ...
    await postService.remove(post1.rows[0].id)
    await postService.remove(post2.rows[0].id)
    await postService.remove(post3.rows[0].id)
})
...
test('Should create a Post', async ()=>{
    ...
    */
    ...
    await postService.remove(response[0].id)
})
...
test('Should update a post', async ()=>{
    ...
    const updatedPost = await postService.get(post.id)
...
test('Should delete a post', async ()=>{
    ...
    const postDeleted = await postService.get(post.id)
